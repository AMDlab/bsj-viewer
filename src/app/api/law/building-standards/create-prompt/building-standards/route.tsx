import { buildingStandardsLawDataPath, buildingStandardsPromptsDir } from '@/const/law';
import { mkdir, readFileSync, writeFileSync } from 'fs';
import { join } from 'path';

export async function GET() {
  try {
    const articles = extractArticles(buildingStandardsLawDataPath);
    mkdir(buildingStandardsPromptsDir, { recursive: true }, (err) => {
      if (err) throw err;
    });
    articles.forEach((article: any, index: number) => {
      const text = format(article['children']);
      writeFileSync(join(buildingStandardsPromptsDir, `${index}.txt`), text, { flag: 'w' });
    });
